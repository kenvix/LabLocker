...
void ntp_init()
{
    ...
    if (timeinfo.tm_year < (2016 - 1900)) {
        ...
        {
            ESP_LOGI(TAG_SNTP, "Add a error for test adjtime");
        ...
        ESP_LOGI(TAG_SNTP, "Time was set, now just adjusting it. Use SMOOTH SYNC method.");
        obtain_time();
        // update 'now' variable with current time
        time(&now);
    }
    ...
    ESP_LOGI(TAG_SNTP, "The current date/time in New York is: %s", strftime_buf);
